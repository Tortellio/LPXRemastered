...
namespace LPXRemastered
{
    public class CommandCost : IRocketCommand
    {
        ...
        public void Execute(IRocketPlayer caller, params string[] command)
        {
            ...
            if (command.Length == 0)
            {
                UnturnedChat.Say(player, LPXRemastered.Instance.Translate("cost_command_usage"));
                return;
            }
            ...
            switch (command[0])
            {
                case "v":
                    ...
                    if (name == null && id == 0)
                    {
                        UnturnedChat.Say(player, LPXRemastered.Instance.Translate("could_not_find",command[1]));
                        return;
                    }
                    ...
                    message = LPXRemastered.Instance.Translate("vehicle_cost_msg", new object[] { name, cost.ToString(), Uconomy.Instance.Configuration.Instance.MoneyName });
                    if (cost <= 0m)
                    {
                        message = LPXRemastered.Instance.Translate("error_getting_cost", new object[] { name });
                    }
                ...
                default:
                    ...
                    if (name == null && id == 0)
                    {
                        UnturnedChat.Say(player, LPXRemastered.Instance.Translate("could_not_find",command[0]));
                        return;
                    }
                    else if (name == null && id != 0)
                    {
                    ...
                    cost = LPXRemastered.Instance.ShopDB.GetItemCost(id);
                    decimal bbp = LPXRemastered.Instance.ShopDB.GetItemBuyPrice(id);
                    ...
                    message = LPXRemastered.Instance.Translate("item_cost_msg", new object[] { name, cost.ToString(), Uconomy.Instance.Configuration.Instance.MoneyName, bbp.ToString(), Uconomy.Instance.Configuration.Instance.MoneyName });
                    if (cost <= 0m)
                        message = LPXRemastered.Instance.Translate("error_getting_cost", new object[] { name });
                    UnturnedChat.Say(player, message);
                    break;
